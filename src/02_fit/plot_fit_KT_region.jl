...
for region_name in keys(regions)
    ...
    for scenario in ["degenerate", "init_omlmax", "init_zero"]
        filename = joinpath(data_path, format("NCAR-fit_KT_region.jl-{}.jld", scenario))
    ...
    fig[:suptitle](
        format("Ocean: {} \n Best fit \$(\\alpha, \\beta) = \$\ndegenerate: ({:.2e}, {:.2f})\ninit_zero: ({:.2e}, {:.2f})\ninit_omlmax: ({})",
            region_name,
            data["degenerate"][region_name]["final_ab"]...,
            data["init_zero"][region_name]["final_ab"]...,
            (init_omlmax_converge) ? format("{:.2e}, {:.2f}", data["init_omlmax"][region_name]["final_ab"]...) : "not converge"
        )
    )
