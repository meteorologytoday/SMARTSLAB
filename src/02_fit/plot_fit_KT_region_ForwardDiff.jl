...
for region_name in keys(regions)
    ...
    for scenario in ["degenerate", "init_omlmax", "init_zero"]
        filename = joinpath(data_path, format("NCAR-fit_KT_region_ForwardDiff.jl-{}.jld", scenario))
    ...
    imgname = joinpath(img_path, format("region_fit-{}-{}_ForwardDiff.png", model_name, region_name))
