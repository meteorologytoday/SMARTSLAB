include("config.jl")
include("../lib/simulate_cores/MLM2L.jl")

using NCDatasets
using .MLM2L

hQ_nc_filename = "stanfit_MLM2L_strong_NCAR_5deg_c4_s1000_w200.nc" 
hQ_nc_filename = normpath(joinpath(data_path, hQ_nc_filename)) 

sim_nc_filename = format("simulate_MLM2L_{}.nc", splitext(basename(hQ_nc_filename))[1])
...
defDim(ds,"time", sim_time)
defDim(ds,"lat", length(lat))
defDim(ds,"lon", length(lon))

defVar(ds, "time", Float64, ("time",))[:] = collect(1:sim_time)
