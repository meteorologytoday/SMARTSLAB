...
model{
    ...
    future_theta[1] = init_theta + (F[1] + Q_extended[1] - (init_theta - theta_d) * we_extended[1]) / h_extended[1] * dt ;
    for(i in 2:trimmed_N) {
        future_theta[i] = future_theta[i-1] + (F[i] + Q_extended[i] - (future_theta[i-1] - theta_d) * we_extended[i]) / h_extended[i] * dt ;
    }
